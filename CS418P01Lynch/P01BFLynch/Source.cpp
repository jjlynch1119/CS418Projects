...
{ 
	if (state.canMoveDown())
	{
		...
		if (!isMember(down, closed) && !isMember(down, open))	//Check if visited already
			open.push_back(PuzzleMove(down, state, MoveType::down));	//Push to queue
	}
	...
	if (state.canMoveLeft())
	{
		...
		if (!isMember(left, closed) && !isMember(left, open))
			open.push_back(PuzzleMove(left, state, MoveType::left));
	}
	...
	if (state.canMoveUp())
	{
		...
		if (!isMember(up, closed) && !isMember(up, open))
			open.push_back(PuzzleMove(up, state, MoveType::up));
	}
	...
	if (state.canMoveRight())
	{
		...
		if (!isMember(right, closed) && !isMember(right, open))
			open.push_back(PuzzleMove(right, state, MoveType::right));
	}
}
...
int main()
{
	cout << " *** P01 Puzzle Uninformed Search - BFS ***\n";
	while (true)
	{
